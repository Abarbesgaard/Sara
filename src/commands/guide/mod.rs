...
mod tests {
    ...
    /// Render a path for embedding in an `sh -c` command string. Verify commands
    /// run under `sh` (git-bash on Windows), where backslashes are escape
    /// characters — a native Windows path like `C:\Users\…\marker` would be
    /// mangled and the redirect target lost. Forward slashes are accepted by
    /// `sh` on every platform, so normalise before interpolating.
    ...
    fn gate_deduplicates_identical_verify_commands() {
        ...
        let cmd = format!("echo x >> {}", sh_path(&marker));
    ...
    fn gate_caches_criteria_already_proven_at_head() {
        ...
        let _ = std::fs::remove_file(&marker);
        let cmd = format!("echo ran >> {}", sh_path(&marker));
