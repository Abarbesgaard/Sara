...
impl SaraServer {
    #[tool(
        description = "[Ordo Termini] Mark a task complete (finalizes its timer, repacks ids, spawns the next recurrence). Errors if the task is blocked unless force=true. A task is done only when its PR is merged — do not call this just because a PR was opened."
    )]
    ...
    #[tool(description = "[Ordo Nexus] Attach a URL (e.g. a PR or issue link) to a task.")]
    ...
    #[tool(
        description = "[Ordo Nexus] Manage task dependencies. action=\"on\": `id` becomes blocked by `other`; \"off\": remove that edge; \"list\": show the task's blockers and what it blocks. `other` is required for on/off."
    )]
    ...
    #[tool(
        description = "[Ordo Termini] Stamp a task's guide as validated against the project's current git HEAD. Errors if the project is not a git repo."
    )]
    ...
    #[tool(
        description = "[Ordo Fundandi] Set task fields non-interactively (never opens the TUI). At least one field is required. `tags` REPLACES the whole tag set."
    )]
    ...
    #[tool(
        description = "[Ordo Termini] Resolve a feedback item by its `feedback_id` (the annotation id from the `feedback`/`info` output — NOT a task id). Optionally cite the `run_id` (from `record_run`) that addressed it."
    )]
    ...
    #[tool(
        description = "[Ordo Recordi] Record an AI/LLM interaction against a task (an audit-trail entry shown in `sara info`'s AI activity section). Returns a `run_id` you can later cite in `resolve`."
    )]
    ...
    #[tool(description = "[Ordo Itineris] Start the time tracker for a task (no-op if it is already active).")]
    ...
    #[tool(
        description = "[Ordo Itineris] Stop the time tracker for a task, recording the session; snapshots a tied branch's changed files if one is set."
    )]
    ...
    #[tool(
        description = "[Ordo Nexus] Remove a link from a task by its sequential link id (shown in `sara info`)."
    )]
    ...
    #[tool(
        description = "[Ordo Recordi] Remove an annotation from a task by its sequential annotation id (shown in `sara info`)."
    )]
    ...
    #[tool(description = "[Ordo Fundandi] Move a task to a different project.")]
