...
4. **Pin it FIRST.** Write (or confirm) tests that fix the current behaviour in place, and **run them green** *before a line is changed*. If none exist, writing them is the first act. `sara step_done` with the passing baseline as `result`.
